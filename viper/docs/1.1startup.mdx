---
id: startup
title: 1.1 Anno 分布式微服务开发框架
sidebar_label: 1.1  Anno 框架概述
slug: /
---
...
- <strong>资源监控</strong>
- <strong>链路追踪</strong>
- <strong>集群接口文档</strong>
- <strong>服务在线安装部署</strong>
- <strong>守护进程，扩容缩容</strong>

![Dashboard](../static/img/home.png)
...
`Anno.Rpc.Client.DynamicProxyGrpc`|接口代理Anno客户端扩展Grpc | [![NuGet](https://img.shields.io/nuget/v/Anno.Rpc.Client.DynamicProxyGrpc.svg?style=flat-square&label=nuget)](https://www.nuget.org/packages/Anno.Rpc.Client.DynamicProxyGrpc/) | ![downloads](https://img.shields.io/nuget/dt/Anno.Rpc.Client.DynamicProxyGrpc.svg)
...
　　AnnoCenter 是一个服务注册中心，主要职责是 发现服务（例如订单服务A、库存服务B）、存储服务配置信息、健康检查、简单键值KV存储。
...
```xml
    ...
    <configuration>
      ...
      <Servers>
        <dc 
        name="Anno.Plugs.TraceService,Anno.Plugs.DLockService,Anno.Plugs.EsLogService" 
        nickname="App001" 
        ip="10.112.93.122"
        port="6659" 
        timeout="20000" 
...
### 服务(AnnoService)
<strong>服务宿主程序，本着约定大于配置的开发原则。</strong><br></br>
插件式开发具体参考：
   <br></br> Packages
      <br></br><code>Anno.Plugs.HelloWorldService</code>
  <br></br>  初始化配置
      <br></br>实现接口： <code>IPlugsConfigurationBootstrap</code>
